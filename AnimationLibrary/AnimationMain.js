...
function doPlayPause()
{
   ...
   if (paused)
   {
      playPauseBackButton.setAttribute("value", "Продолжить");
      if (skipBackButton.disabled == false)
      {
         stepBackButton.disabled = false;      
      }
...
function initCanvas()
{
   ...
   skipBackButton = addControlToAnimationBar("Button", "Исходный массив");
   skipBackButton.onclick = animationManager.skipBack.bind(animationManager);
   stepBackButton = addControlToAnimationBar("Button", "На шаг назад");
   ...
   stepForwardButton = addControlToAnimationBar("Button", "На шаг вперед");
   stepForwardButton.onclick = animationManager.step.bind(animationManager) ;
   skipForwardButton = addControlToAnimationBar("Button", "Результат сортировки");
   ...
   txtNode = document.createTextNode(" Ш:"); 
   ...
   tableEntry = document.createElement("td");
   txtNode = document.createTextNode("       В:"); 
   tableEntry.appendChild(txtNode);
   controlBar.appendChild(tableEntry);
   ...
   heightEntry.size = 4;
   sizeButton = addControlToAnimationBar("Button", "Изменить размер canvas");
